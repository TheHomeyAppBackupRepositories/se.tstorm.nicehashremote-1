...
class NiceHashRigDevice extends homey_1.default.Device {
    constructor() {
        super(...arguments);
        this.lastSync = 0; // When was the last time we synced rig status with NiceHash
        this.lastMined = 0; // When was the last time we mined
        this.benchmarkStart = 0; // benchmark start time
        this.smartMagicNumber = 7; // 7 is the magic number
        this.rollingProfit = 0; // smartMagicNumber minutes rolling profit
    }
    /**
    ...
    async onInit() {
        ...
        if (!this.hasCapability('smart_mode'))
            await this.addCapability('smart_mode');
        if (!this.hasCapability('measure_cost_scarab'))
            await this.addCapability('measure_cost_scarab');
        if (!this.hasCapability('measure_profit_scarab'))
            await this.addCapability('measure_profit_scarab');
        if (!this.hasCapability('meter_cost_scarab'))
            await this.addCapability('meter_cost_scarab');
        if (!this.hasCapability('meter_profit_scarab'))
            await this.addCapability('meter_profit_scarab');
        if (!this.hasCapability('measure_profit_percent'))
            await this.addCapability('measure_profit_percent');
        if (!this.hasCapability('measure_temperature'))
            await this.addCapability('measure_temperature');
        if (!this.hasCapability('measure_load'))
            await this.addCapability('measure_load');
        if (!this.hasCapability('power_mode'))
            await this.addCapability('power_mode');
        if (!this.hasCapability('measure_tariff_limit'))
            await this.addCapability('measure_tariff_limit');
        if (!this.hasCapability('smart_mode_min_profitability'))
            await this.addCapability('smart_mode_min_profitability');
        ...
        this.registerCapabilityListener('onoff', async (value) => {
            ...
            await ((_a = this.niceHashLib) === null || _a === void 0 ? void 0 : _a.setRigStatus(this.getData().id, value));
        });
    ...
    */
    async syncRigDetails() {
        ...
        let temperature = 0;
        let load = 0;
        const details = await ((_a = this.niceHashLib) === null || _a === void 0 ? void 0 : _a.getRigDetails(this.getData().id));
        ...
        if (!details || !details.type || details.type === 'UNMANAGED')
            return;
        const tariff_limit = this.getStoreValue('tariff_limit') || -1;
        if (tariff_limit !== -1)
            this.setCapabilityValue('measure_tariff_limit', tariff_limit).catch(this.error);
        if (details.minerStatus)
            this.setCapabilityValue('status', details.minerStatus).catch(this.error);
        if (details.rigPowerMode)
        ...
        if (details.devices || details.hasV4Rigs) {
            if (details.devices) {
                for (const device of details.devices) {
                    if (device.status.enumName === 'DISABLED' || device.status.enumName === 'OFFLINE')
                        continue;
                ...
                for (const device of details.v4.devices) {
                    ...
                    if (device.mdv && device.mdv.algorithmsSpeed) {
                        for (const algo of device.mdv.algorithmsSpeed) {
                            ...
                            if (!this.algorithms || !this.algorithms[algo.algorithm])
                                await this.getAlgorithms();
                            if (this.algorithms[algo.algorithm])
                                algorithms += (algorithms ? ', ' : '') + this.algorithms[algo.algorithm].title;
                            const r = Number.parseFloat(algo.speed) / 1000000;
                            if (r > 0)
                                mining++;
                            hashrate += r;
                        }
                    ...
                    for (const keypair of device.odv) {
                        if (keypair.key === 'Power usage')
                            powerUsage += Number.parseFloat(keypair.value);
                        if (keypair.key === 'Temperature')
                            temperature = Math.max(temperature, Number.parseFloat(keypair.value));
                        if (keypair.key === 'Load')
                            load += Number.parseFloat(keypair.value);
                    }
                }
            }
            ...
            if (mining === 0) {
                ...
                if (smart_mode
                    ...
                    await ((_b = this.niceHashLib) === null || _b === void 0 ? void 0 : _b.setRigStatus(this.getData().id, true));
                }
                return;
            }
            if (!hashrate) {
            ...
            if (power_tariff && power_tariff_currency) {
                bitcoinRate = (_c = this.niceHashLib) === null || _c === void 0 ? void 0 : _c.getBitcoinRate(power_tariff_currency);
                if (bitcoinRate) {
                    if (mining > 0) {
                    ...
                    if (this.benchmarkStart > 0 && (new Date().getTime() - this.benchmarkStart) > this.smartMagicNumber * 60000) {
                        ...
                        if (this.rollingProfit < smart_mode_min_profitability && profitPct < smart_mode_min_profitability) {
                            ...
                            if (smart_mode) {
                                ...
                                await ((_d = this.niceHashLib) === null || _d === void 0 ? void 0 : _d.setRigStatus(this.getData().id, false));
                            }
                        }
                        else {
                            // Rig is profitable
    ...
    async setSmartModeMinProfitability(minProfitability) {
        var _a;
        await this.setSettings({
            smart_mode_min_profitability: minProfitability,
        });
        // Benchmark if we are not already mining
        await ((_a = this.niceHashLib) === null || _a === void 0 ? void 0 : _a.setRigStatus(this.getData().id, true));
    }
    /**
